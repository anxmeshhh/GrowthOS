...
**[PARTIALLY RESOLVED]** — `SECRET_KEY` is now read from env (see C5). The `.env.prod.example` has been updated with the correct variable names. **⚠️ MANUAL STEPS REQUIRED:** Run `git rm --cached backend/.env.prod` and `git filter-repo --path backend/.env.prod --invert-paths` then rotate ALL secrets (FERNET_KEY, GOOGLE_CLIENT_SECRET, GITHUB_CLIENT_SECRET, GROQ_API_KEY).
...
**⚠️ MANUAL STEPS REQUIRED:** Run `git rm --cached growthos_backup.sql growthos_backup_utf8.sql` then `git filter-repo` to purge from history. Force password resets and revoke all GitHub grants.

**[x] C3 — DRF default permission is `AllowAny`.** `backend/config/settings.py:137-151` sets auth classes
...
**[RESOLVED]** — Added `DEFAULT_PERMISSION_CLASSES` to `REST_FRAMEWORK` in `settings.py`.

**[x] C4 — `LearningPathViewSet` has no `permission_classes` (`views.py:297`).** Because of C3, an
...
**[RESOLVED]** — Added `permission_classes = [IsAuthenticated]` to `LearningPathViewSet` and overrode `update`/`destroy` with ownership + staff checks.

**[x] C5 — Hardcoded `SECRET_KEY` + DB password in source.** `settings.py:13` ships a literal
...
**[RESOLVED]** — `SECRET_KEY` now required from env (raises `ImproperlyConfigured` if missing); `DEBUG` defaults to `False`; `ALLOWED_HOSTS` default no longer contains `*`; DB password default cleared. New `SECRET_KEY` generated and added to local `backend/.env`.

### 🟠 HIGH

**[x] H1 — Google login trusts an unverified access token (`views.py:83-137`).** It calls the userinfo
...
**[RESOLVED]** — `GoogleLoginView` now calls `oauth2.googleapis.com/tokeninfo`, checks `aud == GOOGLE_CLIENT_ID` and `email_verified == true` before proceeding. Generic error returned on exception.

**[x] H2 — GitHub login links accounts by email without checking `verified` (`views.py:139-225`).** Same
takeover class against existing email/password accounts.
*Fix:* require `email['verified'] is True` before linking.
**[RESOLVED]** — `GitHubLoginView` now requires `primary=True AND verified=True` on the email. All `requests` calls got `timeout=10`. Generic errors returned on exception.

**[x] H3 — XP/verification is forgeable.**
...
**[RESOLVED]** — `SubmitQuizView` now grades server-side when `answers` array is sent (preferred path); legacy score/total clamped to [0,100]. `_resolve_topic` now accepts `user` param and enforces path visibility (official / owned / shared). All callers pass `user=request.user`. `PomodoroView` caps duration at 120 min. `VerifyMaterialView` clamps AI score to [0,100] and truncates feedback.

**[x] H4 — JWTs in `localStorage` + 7-day access tokens, no blacklist.** `frontend/src/lib/api-client.ts:3-15`
...
**[PARTIALLY RESOLVED]** — `ACCESS_TOKEN_LIFETIME` reduced to 15 min; `BLACKLIST_AFTER_ROTATION=True`; `rest_framework_simplejwt.token_blacklist` added to `INSTALLED_APPS`. Migration `0029` generated. ⚠️ httpOnly cookie for refresh token requires frontend changes (not yet done).

**[x] H5 — Admin routes have no server-trust boundary on the client and a spoofable super-admin gate.**
...
non-superusers.
**[PARTIALLY RESOLVED]** — `AdminDataExportView` and `AdminRequestDetailView` now check `request.user.is_superuser`. `AdminUserDetailView.patch` now blocks non-superusers from changing `is_staff`. ⚠️ Frontend `beforeLoad` guards on admin routes not yet implemented.

### 🟡 MEDIUM

- **[x] M1 — Zero DB indexes/constraints.** Across `models.py` and all **29 migrations** there is not one
  ...
  **[RESOLVED]** — Added `indexes` to `Contribution` (user, action_type, created_at) and `TopicProgress` (user, status). Added `CheckConstraint` for `points >= 0` on `Contribution`, `ai_score ∈ [0,100]` on `TopicMaterial` and `VerifiedProject`. Migration `0029` generated.
- **[x] M2 — `TopicProgress` lacks `unique_together('user','topic')`.** Every other per-user model has it;
  ...
  **[RESOLVED]** — Added `unique_together = ('user', 'topic')` to `TopicProgress`. Migration `0029` generated. ⚠️ Run `migrate` only after de-duping any existing duplicate rows: `python manage.py shell -c "from core.models import TopicProgress; ..."` if needed.
...
- **[x] M7 — `requests` calls in OAuth/GitHub flows have no `timeout`** (`views.py:93,153,170,178,240,257`)
  → worker-hang DoS. *Fix:* add `timeout=10`.
  **[RESOLVED]** — All `requests.get/post` calls in `GoogleLoginView`, `GitHubLoginView`, and `GitHubConnectView` now have `timeout=10`.
- **[x] M8 — OTP uses non-crypto `random` (`models.py:40`) with no per-email attempt limit.** *Fix:*
  `secrets.randbelow`, add attempt counter + lockout.
  **[PARTIALLY RESOLVED]** — `OTPVerification.generate_otp` now uses `secrets.choice` instead of `random.choices`. ⚠️ Per-email attempt counter + lockout not yet implemented.
...
- **[x] F3 — Hardcoded `localhost:8000`** in `admin.login.tsx:25` and `portfolio.$username.tsx:10` → breaks in
  production.
  **[RESOLVED]** — Both files now use `import.meta.env.VITE_API_BASE_URL` with fallback to localhost.
