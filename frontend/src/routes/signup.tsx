...
function SignupPage() {
    ...
    onSuccess: async (tokenResponse) => {
      try {
        const res = await apiFetch("/auth/google/", {
          method: "POST",
          body: JSON.stringify({ access_token: tokenResponse.access_token, intent: "signup" }),
        });
        if (res.ok) {
  ...
  const handleGithubSignup = () => {
    ...
    const redirectUri = encodeURIComponent(
      `${window.location.origin}/auth/github/callback?intent=signup`,
    );
      ...
      <div className="flex-1 flex flex-col justify-center px-6 sm:px-12 lg:px-16 xl:px-24 py-12">
        <div className="w-full max-w-[400px] mx-auto lg:mx-0 space-y-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm uppercase tracking-widest font-mono text-[#3a3a3a]">
              or verify email
            </span>
            <div className="flex-1 h-px bg-[#1a1a1a]" />
          </div>
